<script lang="ts">
	import { getTeamByEmail, getTeamByName, resetTeamPassword } from '$lib/team';
	import { Button } from '$lib/components/ui/button';
	import { authState } from '$lib/stores/auth';
	import { goto } from '$app/navigation';
	import { Input } from '$lib/components/ui/input';
	import * as Card from '$lib/components/ui/card';
	import * as Table from '$lib/components/ui/table';
	import { Spinner } from '$lib/components/ui/spinner/index.js';
	import { Search, KeyRound, Users as UsersIcon, ExternalLink, ShieldAlert } from '@lucide/svelte';
	import { showSuccess, showError } from '$lib/utils/toast';
	import * as Dialog from '$lib/components/ui/dialog';

	let query = $state('');
	let searchType = $state<'name' | 'email'>('name');
	let loading = $state(false);
	let results = $state<any[]>([]);
	let searched = $state(false);

	let selectedTeam = $state<any>(null);
	let generatedPassword = $state('');
	let showResetDialog = $state(false);
	let resetting = $state(false);

	$effect(() => {
		if (!authState.ready) return;

		if (authState.user?.role !== 'Admin') {
			goto('/admin');
			return;
		}

		if (authState.userMode) {
			goto('/admin/users');
		}
	});

	$effect(() => {
		if (!showResetDialog) {
			generatedPassword = '';
		}
	});

	$effect(() => {
		const q = query;
		if (!q.trim()) {
			results = [];
			searched = false;
			return;
		}

		const timer = setTimeout(() => search(), 400);
		return () => clearTimeout(timer);
	});

	async function search() {
		if (!query.trim()) return;
		loading = true;
		results = [];
		searched = true;

		try {
			const q = query.trim();
			let team: any = null;

			if (searchType === 'email') {
				team = await getTeamByEmail(q);
			} else {
				team = await getTeamByName(q);
			}

			results = team ? [team] : [];
		} catch (err: any) {
			const msg: string = err?.message?.toLowerCase() ?? '';
			const isNotFound =
				msg.includes('not found') || msg.includes('invalid') || msg.includes('404');
			if (!isNotFound) showError(err, 'Failed to search teams');
			results = [];
		} finally {
			loading = false;
		}
	}

	function openReset(team: any) {
		selectedTeam = team;
		generatedPassword = '';
		showResetDialog = true;
	}

	async function handleReset() {
		if (!selectedTeam) return;

		resetting = true;
		try {
			const result = await resetTeamPassword(selectedTeam.id);
			generatedPassword = result?.new_password ?? '';
			showSuccess(`New join password generated for ${selectedTeam.name}.`);
		} catch (err: any) {
			showError(err, 'Failed to reset team password');
		} finally {
			resetting = false;
		}
	}
</script>

<div class="space-y-6">
	<div>
		<h1 class="text-3xl font-bold tracking-tight">Team Management</h1>
		<p class="text-muted-foreground mt-1">Search teams and manage join credentials</p>
	</div>

	<Card.Root>
		<Card.Header>
			<Card.Title class="text-lg">Search Teams</Card.Title>
			<Card.Description>Look up a team by its exact name or email address</Card.Description>
		</Card.Header>
		<Card.Content>
			<form
				class="flex flex-col gap-3 sm:flex-row sm:items-center"
				onsubmit={(e) => {
					e.preventDefault();
					search();
				}}
			>
				<div
					class="bg-muted text-muted-foreground inline-flex shrink-0 items-center gap-1 rounded-lg p-1"
				>
					<button
						type="button"
						class="inline-flex items-center gap-1.5 rounded-md px-4 py-1.5 text-xs font-bold uppercase tracking-wider transition-all {searchType ===
						'name'
							? 'bg-background text-foreground shadow-sm'
							: 'hover:bg-background/50 hover:text-foreground'}"
						onclick={() => {
							searchType = 'name';
							query = '';
							results = [];
							searched = false;
						}}
					>
						Name
					</button>
					<button
						type="button"
						class="inline-flex items-center gap-1.5 rounded-md px-4 py-1.5 text-xs font-bold uppercase tracking-wider transition-all {searchType ===
						'email'
							? 'bg-background text-foreground shadow-sm'
							: 'hover:bg-background/50 hover:text-foreground'}"
						onclick={() => {
							searchType = 'email';
							query = '';
							results = [];
							searched = false;
						}}
					>
						Email
					</button>
				</div>
				<div class="relative flex-1">
					{#if loading}
						<Spinner class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2" />
					{:else}
						<Search
							class="text-muted-foreground absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2"
						/>
					{/if}
					<Input
						bind:value={query}
						placeholder={searchType === 'name'
							? 'Exact team name (e.g. team rocket)'
							: 'Team email (e.g. team@ctf.io)'}
						class="pl-9"
					/>
				</div>
				<Button type="submit" disabled={loading} class="shrink-0">Search</Button>
			</form>
		</Card.Content>
	</Card.Root>

	{#if results.length > 0}
		<Card.Root class="overflow-hidden border-0 shadow-sm">
			<Card.Content class="p-0">
				<div class="relative mx-4 overflow-auto sm:mx-6">
					<Table.Root>
						<Table.Header class="bg-transparent [&_tr]:border-b-0">
							<Table.Row class="border-none hover:bg-transparent">
								<Table.Head
									class="text-muted-foreground/70 w-[80px] bg-transparent text-[10px] font-bold uppercase tracking-wider"
									>ID</Table.Head
								>
								<Table.Head
									class="text-muted-foreground/70 bg-transparent text-[10px] font-bold uppercase tracking-wider"
									>Team</Table.Head
								>
								<Table.Head
									class="text-muted-foreground/70 bg-transparent text-[10px] font-bold uppercase tracking-wider"
									>Country</Table.Head
								>
								<Table.Head
									class="text-muted-foreground/70 bg-transparent text-right text-[10px] font-bold uppercase tracking-wider"
									>Actions</Table.Head
								>
							</Table.Row>
						</Table.Header>
						<Table.Body>
							{#each results as team (team.id)}
								<Table.Row class="hover:bg-muted/50 group border-none transition-colors">
									<Table.Cell class="font-mono text-xs">{team.id}</Table.Cell>
									<Table.Cell>
										<div class="flex flex-col">
											<span class="text-sm font-medium">{team.name}</span>
											<span class="text-muted-foreground text-xs">
												{team.email || 'No email set'}
											</span>
										</div>
									</Table.Cell>
									<Table.Cell>
										<span class="text-sm">{team.country || 'N/A'}</span>
									</Table.Cell>
									<Table.Cell class="text-right">
										<div class="flex items-center justify-end gap-1">
											<a
												href="/team/{team.id}"
												target="_blank"
												rel="noopener noreferrer"
												class="text-muted-foreground hover:text-foreground hover:bg-muted inline-flex h-8 items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs transition-colors"
											>
												<ExternalLink class="h-3.5 w-3.5" />
												Profile
											</a>
											<Button
												variant="ghost"
												size="sm"
												class="text-muted-foreground hover:text-foreground h-8 gap-2 transition-colors"
												onclick={() => openReset(team)}
											>
												<KeyRound class="h-3.5 w-3.5" />
												Reset
											</Button>
										</div>
									</Table.Cell>
								</Table.Row>
							{/each}
						</Table.Body>
					</Table.Root>
				</div>
			</Card.Content>
		</Card.Root>
	{:else if searched && query && !loading}
		<div
			class="bg-muted/20 flex flex-col items-center justify-center rounded-lg border border-dashed py-12"
		>
			<UsersIcon class="text-muted-foreground mb-2 h-10 w-10 opacity-20" />
			<p class="text-muted-foreground text-sm">
				No team found with {searchType} <span class="text-foreground font-medium">"{query}"</span>
			</p>
		</div>
	{/if}
</div>

<Dialog.Root bind:open={showResetDialog}>
	<Dialog.Content class="sm:max-w-[425px]">
		<Dialog.Header>
			<Dialog.Title class="flex items-center gap-2">
				<ShieldAlert class="text-destructive h-5 w-5" />
				Regenerate Join Password
			</Dialog.Title>
			<Dialog.Description>
				Create a new join password for <span class="text-foreground font-bold"
					>{selectedTeam?.name}</span
				>. The current password will stop working.
			</Dialog.Description>
		</Dialog.Header>
		<div class="grid gap-4 py-4">
			{#if generatedPassword}
				<div class="space-y-2">
					<p class="text-sm font-medium">New password</p>
					<p class="bg-muted cursor-text select-all rounded-md px-3 py-2 font-mono text-sm">
						{generatedPassword}
					</p>
				</div>
			{/if}
		</div>
		<Dialog.Footer>
			<Button variant="ghost" onclick={() => (showResetDialog = false)}>Close</Button>
			<Button variant="destructive" onclick={handleReset} disabled={resetting}>
				{#if resetting}
					<Spinner class="mr-2 h-4 w-4" />
				{/if}
				{generatedPassword ? 'Regenerate Password' : 'Generate Password'}
			</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
